...
- `csv_agent.py` loads the trained adapter from `./my_kcc_agent` and answers directly from the fine-tuned model.
...
The previous version of this file overstated a few things. The code does not guarantee factual recall, hallucination-free answers, or production-grade deployment by itself. It is better described as a local benchmarking and training prototype with a fine-tuned local inference helper.
...
- The inference helper depends on the saved adapter directory `./my_kcc_agent`.
````
